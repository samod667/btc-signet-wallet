...
from test_framework.util import PortSeed
...
T.setup(
    ...
    port_seed = PORT_SEED,
    # TODO: anything added here needs to also go in bitcoin.conf
    extra_args=[[
...
shell.log.info(f"node0 RPC port: {node}\n")
...
with open(f"{CONF_DIR / 'bitcoin.conf'}", "w") as f:
    ...
    f.write("[signet]\n")
    f.write(f"signetchallenge={signet_challenge}\n")
    f.write(f"addnode={EXTERNAL_IP}:{P2P_PORT}\n")
...
if REGTEST:
    ...
    # TODO: make a 1-block mining function and call it after evey tx batch to speed up
    miner = f"{repo / 'contrib' / 'signet' / 'miner'}"
    grinder = f"{repo / 'build' / 'src' / 'bitcoin-util'}"
    cmd = [
        sys.executable,
        miner,
    ...
    while True:
        ...
        try:
            ...
            if height == old_height:
                # We're still waiting on a new block before checking wallet balances
                sleep(200)
                continue
            for wallet in wallets:
